...
def check_regular_guard_witness(seed):
    """Verify that the regular localization is nonempty at one exact seed."""
    product = sp.symbols("product", nonzero=True)
    vertical_coordinate = sp.symbols("vertical_coordinate", nonzero=True)
    horizontal = fixed_data(seed, product, x)
    transposed_seed = (seed[0], seed[2], seed[1], seed[3])
    vertical_base = fixed_data(transposed_seed, product, vertical_coordinate)
    kappa = sp.cancel(
        vertical_base["delta"] * horizontal["c6"]
        / (horizontal["delta"] * vertical_base["c6"])
    )
    vertical_product = sp.cancel(
        (vertical_base["U"] - kappa * (2 * product - horizontal["U"])) / 2
    )
    vertical = fixed_data(
        transposed_seed, vertical_product, vertical_coordinate
    )
    product_modulus = sp.Poly(horizontal["product_quadratic"], product)
    if not product_modulus.is_irreducible:
        raise AssertionError("specialized product quadratic is reducible")

    def nonzero_in_product_field(expression, label):
        numerator = sp.together(expression).as_numer_denom()[0]
        remainder = sp.rem(sp.Poly(numerator, product), product_modulus)
        if remainder.is_zero:
            raise AssertionError(f"regular guard vanished: {label}")

    a, b, c, d = map(sp.Rational, seed)
    E = sp.Matrix([[1, 1, 1], [1, a, b], [1, c, d]])
    E_sharp = sp.Matrix(
        [[1 / E[column, row] for column in range(3)] for row in range(3)]
    )
    scalar_guards = {
        "det(E)": E.det(),
        "det horizontal Gram": (6 * sp.eye(3) - E * E_sharp).det(),
        "det vertical Gram": (6 * sp.eye(3) - E_sharp * E).det(),
        "horizontal c6": horizontal["c6"],
        "vertical c6": vertical["c6"],
        "horizontal delta": horizontal["delta"],
        "vertical delta": vertical["delta"],
    }
    for label, value in scalar_guards.items():
        nonzero_in_product_field(value, label)

    algebraic_guards = {
        "horizontal cubic discriminant": sp.discriminant(
            horizontal["cubic"].as_expr(), x
        ),
        "vertical cubic discriminant": sp.discriminant(
            vertical["cubic"].as_expr(), vertical_coordinate
        ),
        "horizontal companion resultant": sp.resultant(
            horizontal["cubic"].as_expr(), horizontal["B"].as_expr(), x
        ),
        "vertical companion resultant": sp.resultant(
            vertical["cubic"].as_expr(),
            vertical["B"].as_expr(),
            vertical_coordinate,
        ),
    }
    for label, value in algebraic_guards.items():
        nonzero_in_product_field(value, label)

    print("PASS all regular localization guards are nonzero at (2,3,5,7)")


def main():
    seed = (2, 3, 5, 7)
    check_regular_guard_witness(seed)
